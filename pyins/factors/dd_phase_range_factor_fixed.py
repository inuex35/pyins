"""Double-difference phase range factor with fixed ambiguity."""
...
class DDPhaseRangeFactorFixed:
    """Double-difference phase range factor that uses fixed ambiguities."""
    ...
    def __init__(
        ...
        if reference_llh is None:
            raise ValueError("reference_llh must be provided for DDPhaseRangeFactorFixed")
    ...
    def create_factor(self) -> gtsam.CustomFactor:
        def error_func(this: gtsam.CustomFactor, values: gtsam.Values, H: list) -> np.ndarray:
            ...
            residual = dd_measurement_m - (dd_range + self.fixed_ambiguity_meters + bias_sum)
...
__all__ = ['DDPhaseRangeFactorFixed']
